...
// ----------------------------------------------------------------------------
// NEW: Prefix parsing and matching
// ----------------------------------------------------------------------------
...
Int BTCCollider::ClampKey(Int &key) {
  ...
  if (result.IsGreater(&keyRangeWidth))
...
BTCCollider::BTCCollider(Secp256K1 *secp, bool useGpu, bool stop, std::string outputFile, std::string workFile,
                         ...
                         std::string targetPrefix, std::string rangeStartHex, std::string rangeEndHex) {
  ...
  if (useRange) {
    ...
    printf("Key range width : 2^%.2f\n", log2(pow(2.0, (double)keyRangeWidth.GetBitLength())));
  }
...
void BTCCollider::Check(std::vector<int> gpuId, std::vector<int> gridSize) {
  ...
  // NEW: Test prefix matching and key range clamping
  if (usePrefix) {
    printf("Prefix filter test:\n");
    int matchCount = 0;
    for (int t = 0; t < CPU_GRP_SIZE; t++) {
      if (MatchesPrefix(&xc[t])) matchCount++;
    }
    printf("  %d / %d hashes matched prefix\n", matchCount, CPU_GRP_SIZE);
  }

  if (useRange) {
    printf("Key range test:\n");
    Int testKey;
    Rand(&seed, &testKey);
    Int clamped = ClampKey(testKey);
    bool inRange = (clamped.IsGreaterOrEqual(&keyRangeStart) &&
                    keyRangeEnd.IsGreaterOrEqual(&clamped));
    printf("  Random key clamped to range: %s\n", inRange ? "YES" : "FAIL");
  }
...
void BTCCollider::InitKey(TH_PARAM *p) {
  ...
  // NEW: If range is set, clamp the base private key into range
  if (useRange) {
    k = ClampKey(k);
  }
...
Int BTCCollider::GetPrivKey(hash160_t x) {
  ...
  // NEW: Clamp the derived private key into range
  if (useRange) {
    k = ClampKey(k);
  }
...
Point BTCCollider::Add(Point &p1, int n, uint16_t h) {
  ...
  Int u; Int v; Int u1; Int v1; Int vs2; Int vs3;
  Int us2; Int a; Int us2w; Int vs2v2; Int vs3u2; Int _2vs2v2;
  Point r;
...
void BTCCollider::FindCollisionCPU(TH_PARAM *ph) {
  ...
  while (!endOfSearch) {
    ...
    // Random walk (batched Montgomery inversion)
    ...
    if (!endOfSearch) {
      ...
      for (int g = 0; g < CPU_GRP_SIZE; g++) {
        if (IsDP(ph->y[g])) {
          ...
          // NEW: If prefix filtering is active, check before storing in hash table
          if (usePrefix && !MatchesPrefix(&ph->y[g])) {
            // DP doesn't match target prefix — reset this walk
  ...
  while(!endOfSearch) {
    ...
    if (hashFound.size() > 0) {
      ...
      for (int i = 0; !endOfSearch && i < (int)hashFound.size(); i++) {
        ...
        // NEW: If prefix filtering is active, check GPU results before storing
        if (usePrefix && !MatchesPrefix((hash160_t *)(hashFound[i].h2))) {
          continue;  // Skip non-matching DPs from GPU
        }
...
void BTCCollider::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  for (int Try = 1; Try <= MAXTRY; Try++) {
    ...
    // NEW: Print prefix/range info at search start
    if (usePrefix) {
      printf("Prefix filter active: ");
      for (int i = 0; i < (prefixLength + 1) / 2; i++)
        printf("%02X", prefixBytes[i]);
      printf(" (%d bits)\n", prefixBitLen);
    }
...
#define FILTER_SIZE 8
    ...
    if (useRange) {
      ...
      bool k2inRange = (k2.IsGreaterOrEqual(&keyRangeStart) && keyRangeEnd.IsGreaterOrEqual(&k2));
      printf("Key1 in range: %s\n", k1inRange ? "YES" : "NO");
      printf("Key2 in range: %s\n", k2inRange ? "YES" : "NO");
    }
